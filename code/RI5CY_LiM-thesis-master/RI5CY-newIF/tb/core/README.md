...
it. Instructions added below on 
...
Installation Newlib cross-compiler [RISC-V GNU Toolchain]
-----------------------
To build the Newlib cross-compiler, pick an install path.  If you choose,
say, `/opt/riscv`, then create a sub-folder `/opt/riscv/bin` to your `PATH` now.  Then, simply
run the following command in any other paths:
...
You should now be able to use riscv32-unknown-elf-gcc and its cousins.
